...
if (isProduction) {
  ...
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    throw new Error('SMTP_USER and SMTP_PASS must be set in production (password reset depends on email)');
  }
}
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      // ~29 inline onclick/onsubmit handlers remain across the views. Blocking
...
app.use((err, req, res, next) => {
  ...
  if (err instanceof multer.MulterError || err.message === 'Only image files are allowed') {
    const key = err.code === 'LIMIT_FILE_SIZE' ? 'file_too_large'
      : err.code === 'LIMIT_FILE_COUNT' ? 'too_many_photos'
      : 'upload_type_error';
...
initDB().then(() => {
  ...
  const shutdown = (signal) => {
    ...
    setTimeout(() => { saveDB(); process.exit(1); }, 8000).unref();
  };
