(function () {
  ...
  var TOAST_MESSAGES = {
    ...
    listing_submitted:
      'Listing submitted for review. Admins typically approve within 48 hours. You can keep editing it in the meantime.',
    listing_updated:
      'Changes saved and live. Students browsing KejaHub see your updated listing right away.',
    listing_resubmitted:
      'Changes saved and sent for review. Because this listing was previously rejected, an admin will take another look within 48 hours.',
    ...
    reset_link_sent:
      'If that email is registered, we have sent a reset link. It expires in 1 hour. Check your spam folder if it does not arrive.',
    ...
    unauthorized:
      "You don't have permission to do that. If you think this is wrong, try logging out and back in.",
    session_expired: 'Your session expired for security. Please log in again to continue.',
    file_too_large:
      'That image is over 5 MB. Compress or resize it, then upload again. Nothing else on your form was lost.',
    upload_type_error:
      'Only JPEG, PNG, and WebP images are accepted. Convert the file and try again.',
    too_many_photos:
      'A listing can hold up to 10 photos. Remove a few existing ones, then add the new ones.',
    upload_failed:
      'Your photos could not be uploaded. Check your connection and try again. Your listing details were not changed.',
    listing_not_found: 'That listing no longer exists, or it is not yours to edit.',
    booking_not_found:
      'That booking request no longer exists. Refresh the page to see the latest requests.',
    invalid_booking_status:
      'That is not a valid response to a booking. Please choose Accept or Decline.',
    already_requested:
      "You already have a pending request for this house. Watch My Bookings for the landlord's reply.",
    already_reviewed:
      'You have already reviewed this listing. You can only leave one review per house.',
    ...
    wrong_password:
      'Your current password is incorrect. Try again, or use "Forgot password" to reset it.',
  ...
  var FALLBACK =
    'Something went wrong. Please try again. If it keeps happening, contact support@kejahub.com.';
  ...
  // The browser only needs window.Toast above. This makes the message table
  // reachable from a test runner as well, so the copy can be asserted directly
  // rather than scraped back out of the DOM.
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { TOAST_MESSAGES: TOAST_MESSAGES, FALLBACK: FALLBACK, Toast: window.Toast };
  }
})();
